...
class RegisterUser(tk.Frame):
    def __init__(self,master, username):
        ...
        form_heading  = ttk.Label(form_frame,text = "Register as User", font="Montserrat 20 bold",foreground='#4267b2')
        ...
        address = tk.Label(form_frame, text="Address")
        ...
        def registerUser():
            ...
            database.createCustomer((username_field.get(), name_field.get(), gender_field.get(),
                    email_field.get(), phoneNum_field.get(), address_field.get(), password_field.get()))
            master.frameSwitcher(CustomerDashboard, username_field.get())
            

        submit = tk.Button(form_frame, text="Submit", command=registerUser)
